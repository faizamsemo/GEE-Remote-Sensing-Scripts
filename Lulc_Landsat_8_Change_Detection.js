/**************************************************************
 * SCRIPT 2: LANDSAT 8 ONLY — TWO YEARS, INDEPENDENT TRAINING
 * Example pairs:
 *   2015 -> 2020
 **************************************************************/
...
var trainingAssets = {
  ...
  2020: {
    builtup: Builtup_2020,
    cropland: Cropland_2020,
    forest: Forest_2020,
    degraded: Degraded_2020,
    sandriver: Sandriver_2020
}
};
...
function compositeL8_HoleFilled(year) {
  ...
  var wide = buildCompositeL8(
    ee.Date.fromYMD(year - WIDE_YEARS, 1, 1),
    ee.Date.fromYMD(year + WIDE_YEARS, 12, 31).advance(1, 'day'),
    maskL8Relaxed, 90
  );
...
function trainAndAssessYear(image, year, sensorTag) {
  ...
  var trainFC = split.filter(ee.Filter.lt('rand', 0.8));
  var validFC = split.filter(ee.Filter.gte('rand', 0.8));
...
// 8) RUN BOTH YEARS
// ==============================
var img1 = compositeL8_HoleFilled(YEAR1);
var img2 = compositeL8_HoleFilled(YEAR2);

var result1 = trainAndAssessYear(img1, YEAR1, 'L8');
...
// 11) EXPORTS
// ==============================
var outFolder = 'L8_LULC_' + YEAR1 + '_' + YEAR2;
var accuracyAll = result1.accuracy.merge(result2.accuracy);

Export.image.toDrive({
  image: cls1.toInt16(),
  description: 'L8_LULC_' + YEAR1,
  folder: outFolder,
  region: roi,
  scale: SCALE,
  maxPixels: 1e13,
...
Export.table.toDrive({
  ...
  description: 'L8_Transitions_' + YEAR1 + '_' + YEAR2,
  folder: outFolder,
  fileFormat: 'CSV'
});
...
Export.table.toDrive({
  ...
  description: 'L8_Accuracy_' + YEAR1 + '_' + YEAR2,
  folder: outFolder,
  fileFormat: 'CSV'
});